...
export const error = (type: unknown, errorCode: ErrorCode, message: unknown) =>
  `${chalk.redBright(`BlockBuild${type}`)} (${errorCode}): ${message}`;
export const CLIError = (errorCode: ErrorCode, message: unknown) =>
  error("CLIError", errorCode, message);
export const RuntimeError = (
  errorCode: ErrorCode,
  filterId: unknown,
  message: unknown
) => error("RuntimeError", errorCode, `In \`${filterId}\`\n\t${message}`);
export const InternalError = (errorCode: ErrorCode, message: unknown) =>
  error("InternalError", errorCode, message);
export const UncaughtError = (errorCode: ErrorCode, err: Error) =>
  error("UncaughtError", errorCode, err);
...
ZodError.tryParseSchema = (
  ...
  if (!result.success)
    throw ZodError(errorCode, errorMessage, result.error.issues[0]);
  return result.data;
};
