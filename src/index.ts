...
export async function build(config: types.IConfigEvaluated, buildFlags: any) {
	...
	for (const filterToExecute of config.filters) {
		...
		try {
			...
			throw errors.RuntimeError(
				errors.ErrorCode.RuntimeMainFunctionThrew,
				filterToExecute.id,
				`Main function threw an error.\n\t${err}`
			);
		}
	}

	if (!buildFlags.production && !buildFlags.package) {
		const outBPPath = path.join(config.outPath, "BP");
		const outRPPath = path.join(config.outPath, "RP");
		...
		if (config.packs.includes("BP"))
			(async () => {
				...
				await fs.cp(outBPPath, destPath, { recursive: true });
				console.log(`Copied \`${outBPPath}\` to \`${destPath}\`. Use \`--production\` to disable this.`);
			})();
		...
		if (config.packs.includes("RP"))
			(async () => {
				...
				await fs.rm(destPath, { recursive: true, force: true });
				await fs.cp(outRPPath, destPath, { recursive: true });
				console.log(`Copied \`${outRPPath}\` to \`${destPath}\`. Use \`--production\` to disable this.`);
			})();
	}
